...
resource "aws_subnet" "prsnet" {
  count = length(var.availability_zone)
  vpc_id = aws_vpc.vpc.id
  depends_on = [aws_vpc_ipv4_cidr_block_association.vpc_sec]
  availability_zone = var.availability_zone[count.index]
...
resource "aws_ram_principal_association" "principal" {
  principal          = "016690563450"  # destination account
  resource_share_arn = aws_ram_resource_share.share.arn
}

resource "aws_ram_resource_association" "pusubnet1" {
  resource_arn       = aws_subnet.pusnet[0].arn  # source account
  resource_share_arn = aws_ram_resource_share.share.arn
}

resource "aws_ram_resource_association" "pusubnet2" {
  resource_arn       = aws_subnet.pusnet[1].arn  # source account
  resource_share_arn = aws_ram_resource_share.share.arn
}
resource "aws_ram_resource_association" "prsubnet1" {
  resource_arn = aws_subnet.prsnet[0].arn
  resource_share_arn = aws_ram_resource_share.share.arn
}

resource "aws_ram_resource_association" "prsubnet2" {
  resource_arn = aws_subnet.prsnet[1].arn
  resource_share_arn = aws_ram_resource_share.share.arn
}